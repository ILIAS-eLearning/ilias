...
class ilObjTestSettingsGeneralGUI extends ilTestSettingsGUI
{
    ...
    protected function getSettingsTemplateMessageHTML(): string
    {
        ...
        if ($this->settingsTemplate->isAvailable()) {
            ...
            $message = sprintf($this->lng->txt('test_using_template'), $title, $link);
        } else {
            $message = sprintf($this->lng->txt('test_using_removed_template'), $link);
        }

        $type = $this->settingsTemplate->isAvailable() ? 'info' : 'failure';
        return '<div style="margin-top:10px">' . ilUtil::getSystemMessageHTML($message, $type) . '</div>';
    }
