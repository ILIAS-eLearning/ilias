...
class ilObjUserFolderGUI extends ilObjectGUI
{
    ...
    public function saveGlobalUserSettingsObject(string $action = ''): void
    {
        ...
        if ($selected['default_hits_per_page'] ?? false) {
            $this->ilias->setSetting(
                'hits_per_page',
        ...
        $this->ilias->setSetting(
            'mail_incoming_mail',
            (string) ($selected['default_mail_incoming_mail'] ?? '0')
        );
        $this->ilias->setSetting(
            'chat_osc_accept_msg',
            (string) ($selected['default_chat_osc_accept_msg'] ?? 'n')
        );
        $this->ilias->setSetting(
            'chat_broadcast_typing',
            (string) ($selected['default_chat_broadcast_typing'] ?? 'n')
        );
        $this->ilias->setSetting(
            'bs_allow_to_contact_me',
            (string) ($selected['default_bs_allow_to_contact_me'] ?? 'n')
        );
        $this->ilias->setSetting(
            'hide_own_online_status',
            (string) ($selected['default_hide_own_online_status'] ?? 'n')
        );
