...
class ilObjLearningSequenceLearnerGUI
{
    ...
    private function getWrappedHTML(array $components): string
    {
        array_unshift(
            $components,
            $this->ui_factory->legacy()->legacyContent('<div class="ilLSOLearnerView">')
        );
        $components[] = $this->ui_factory->legacy()->legacyContent('</div>');
    ...
    private function getMainContent(string $cmd): array
    {
        ...
        if ($cmd === self::CMD_STANDARD) {
            if ($this->intro === '') {
                $contents[] = $this->ui_factory->legacy()->legacyContent($this->settings->getAbstract());
                $img = $this->settings->getAbstractImage();
                if ($img) {
                    $contents[] = $this->ui_factory->image()->responsive($img, '');
                }
            } else {
                $contents[] = $this->ui_factory->legacy()->legacyContent($this->intro);
            }
        }

        if ($cmd === self::CMD_EXTRO) {
            if ($this->extro === '') {
                $contents[] = $this->ui_factory->legacy()->legacyContent($this->settings->getExtro());
                $img = $this->settings->getExtroImage();
                if ($img) {
                    $contents[] = $this->ui_factory->image()->responsive($img, '');
                }
            } else {
                $contents[] = $this->ui_factory->legacy()->legacyContent($this->extro);
            }
        }
        return $contents;
    }
