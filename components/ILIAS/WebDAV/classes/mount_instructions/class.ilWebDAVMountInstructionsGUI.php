...
class ilWebDAVMountInstructionsGUI
{
    ...
    public function buildGUIFromGivenMountInstructions(array $mount_instructions, bool $render_async = false): string
    {
        ...
        $js_function_legacy = $f->legacy()->legacyContent('<script>'
        ...
        if (count($mount_instructions) === 1) {
            $content = $f->legacy()->legacyContent("<div class='instructions'>" . array_shift($mount_instructions) . "</div>");
        ...
        foreach ($mount_instructions as $title => $text) {
            ...
            $legacy = $f->legacy()->legacyContent("<div id='$title' class='instructions' $hidden>$text</div>")
        ...
        $header_components = [
            $f->legacy()->legacyContent("<div class='webdav-view-control'>"),
            $view_control,
            $f->legacy()->legacyContent("</div>"),
            $js_function_legacy];
