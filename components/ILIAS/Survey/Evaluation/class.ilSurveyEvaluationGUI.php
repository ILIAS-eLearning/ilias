    ...
    public function evaluation(
        ...
        if (!$this->object->get360Mode() || $appr_id) {
                ...
                if ($details) {
                    ...
                    if ($qdata["questionblock_id"] &&
                        ...
                        if ($qblock["show_blocktitle"]) {
                            $listing->node(
                                $this->ui->factory()->legacy()->legacyContent($qdata["questionblock_title"]),
                                "q" . $qdata["questionblock_id"]
                            );
                        } else {
                            $listing->node(
                                $this->ui->factory()->legacy()->legacyContent(""),
                                "q" . $qdata["questionblock_id"]
                            );
                        }
            ...
            if ($details) {
                ...
                //TABLE OF CONTENTS
                $panel_toc = $ui_factory->panel()->standard("", $ui_factory->legacy()->legacyContent($toc_tpl->get()));
