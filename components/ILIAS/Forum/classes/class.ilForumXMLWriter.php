...
class ilForumXMLWriter extends ilXmlWriter
{
    ...
    private ?string $absolute_root_export_path = null;
    ...
    {
        $path = str_replace('\\', '/', $a_rel);
        $segments = explode('/', $path);
        array_shift($segments);
        $a_rel = implode('/', $segments);

        $this->relative_component_path = $a_rel;
        $this->absolute_root_export_path = rtrim($a_abs, '/');
    }

    public function getComponentPath(): string
    {
        return $this->absolute_root_export_path . '/' . $this->relative_component_path;
    }
    ...
    public function start(): bool
    {
        ...
        ilFileUtils::makeDirParents($this->getComponentPath());
        ilFileUtils::makeDir($this->getComponentPath() . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY);
        ilFileUtils::makeDir($this->getComponentPath() . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY . '/objects');
        ...
        while ($row = $ilDB->fetchObject($res)) {
            ...
            while ($rowPost = $ilDB->fetchObject($resPosts)) {
                ...
                $this->xmlElement('isAuthorModerator', null, (string) $rowPost->is_author_moderator);
                ...
                foreach ($mobs as $mob) {
                    ...
                    if (ilObjMediaObject::_exists($mob)) {
                        ...
                        $imgattrs = [
                            'label' => $moblabel,
                            'uri' => $this->relative_component_path . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY . '/objects/' . 'il_' . IL_INST_ID . '_mob_' . $mob . '/' . $mob_obj->getTitle()
                        ];

                        $this->xmlElement('MediaObject', $imgattrs, null);
                        $mob_obj->exportFiles($this->getComponentPath() . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY);
                    }
                }
                if ($media_exists) {
                ...
                foreach ($tmp_file_obj->getFilesOfPost() as $file) {
                    ...
                    copy($file['path'], $this->getComponentPath() . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY . '/' . basename($file['name']));
                    $relative_path = $this->relative_component_path . '/' . self::LEGACY_RESOURCE_SUB_DIRECTORY . '/' . basename($file['name']);
                    $this->xmlElement('Content', null, $relative_path);
