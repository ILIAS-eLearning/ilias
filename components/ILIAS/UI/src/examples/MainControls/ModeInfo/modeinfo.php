...
function modeinfo(): string
{
    ...
    return $renderer->render([
        ...
        $f->legacy()->legacyContent('<p><b>press the link above to init a page with Mode Info</b></p><p><br/></p>'),
        $f->divider()->horizontal()
    ]);
}
...
function renderModeInfoFullscreenMode(\ILIAS\DI\Container $dic)
{
    ...
    $panel_content = $f->legacy()->legacyContent("Mode Info is Active");
    $slate = $f->mainControls()->slate()->legacy(
        "Mode Info Active",
        $f->symbol()->glyph()->notification(),
        $f->legacy()->legacyContent("Things todo when special Mode is active")
    );
