...
function nested()
{
    ...
    $vertical = $ui_factory->layout()->alignment()->vertical(
        $ui_factory->legacy()->legacyContent('<div class="example_block fullheight red">The block above.</div>'),
        $evenly,
        $ui_factory->legacy()->legacyContent('<div class="example_block fullheight red">The block below.</div>')
    );
