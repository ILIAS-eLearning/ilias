...
function show_multi_step_modal()
{
    ...
    if ($page == "") {
        $modal = $f->modal()->roundtrip("Modal Title", $f->legacy()->legacyContent("b"));
        ...
        $modal = null;
        if ($page == "login") {
            $legacy = $f->legacy()->legacyContent("<p>The Login Page</p>");
            $modal = $f->modal()->roundtrip("Login", [$legacy])->withActionButtons([$button1, $button2]);
        }
        if ($page == "register") {
            $legacy = $f->legacy()->legacyContent("<p>The Registration Page</p>");
