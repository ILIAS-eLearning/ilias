    ...
    public function getToolsForContextStack(CalledContexts $called_contexts): array
    {
        ...
        if ($additional_data->is(self::SHOW_SKILL_TREE, true)) {
            ...
            $tools[] = $this->factory->tool($this->identification_provider->contextAwareIdentifier("tree"))
                ->withTitle($title)
                ->withSymbol($icon)
                ->withContentWrapper(function () use ($tree_id): LegacyContent {
                    return $this->dic->ui()->factory()->legacy()->legacyContent($this->getSkillTree($tree_id));
                });
        }
        ...
        if ($additional_data->is(self::SHOW_TEMPLATE_TREE, true)) {
            ...
            $tools[] = $this->factory->tool($this->identification_provider->contextAwareIdentifier("tree"))
                ...
                ->withSymbol($icon)
                ->withContentWrapper(function () use ($tree_id): LegacyContent {
                    return $this->dic->ui()->factory()->legacy()->legacyContent($this->getTemplateTree($tree_id));
                });
        }
        return $tools;
    }
